...
transition(pda(q_idle, [U|Rest]), tick, pda(q_idle, [NewU|Rest])) :-
    atom_concat('U', N_str, U), atom_number(N_str, N), N < 9,
    integer_to_recollection(N, RecN),
    grounded_arithmetic:successor(RecN, RecNewN),
    recollection_to_integer(RecNewN, NewN),
...
transition(pda(q_inc_tens, [T|Rest]), '', pda(q_idle, ['U0', NewT|Rest])) :-
    atom_concat('T', N_str, T), atom_number(N_str, N), N < 9,
    integer_to_recollection(N, RecN),
    grounded_arithmetic:successor(RecN, RecNewN),
    recollection_to_integer(RecNewN, NewN),
...
transition(pda(q_inc_hundreds, [H|Rest]), '', pda(q_idle, ['U0', 'T0', NewH|Rest])) :-
    atom_concat('H', N_str, H), atom_number(N_str, N), N < 9,
    integer_to_recollection(N, RecN),
    grounded_arithmetic:successor(RecN, RecNewN),
    recollection_to_integer(RecNewN, NewN),
...
stack_to_int([U, T, H, '#'], Value) :-
    ...
    integer_to_recollection(T_val, RecT_val),
    integer_to_recollection(10, Rec10),
    grounded_arithmetic:multiply_grounded(RecT_val, Rec10, RecTens),
    integer_to_recollection(H_val, RecH_val),
    integer_to_recollection(100, Rec100),
    grounded_arithmetic:multiply_grounded(RecH_val, Rec100, RecHundreds),
    grounded_arithmetic:add_grounded(RecU, RecTens, RecPartial),
    grounded_arithmetic:add_grounded(RecPartial, RecHundreds, RecValue),

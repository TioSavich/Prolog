...
test_prover_basics :-
    ...
    run_test('Identity rule', (
        embodied_prover:proves([a] => [a], 10, _, _)
    )),

    run_test('Explosion from incoherence', (
        embodied_prover:proves([a, neg(a)] => [b], 10, _, _)
    )),

    run_test('Left negation (double negation elimination)', (
        embodied_prover:proves([neg(neg(a))] => [a], 10, _, Proof),
        Proof \= erasure(_)
    )),

    run_test('Resource tracking', (
        embodied_prover:proves([a] => [a], 100, R_Out, _),
        R_Out < 100
    )),

    run_test('Resource exhaustion', (
        (catch(embodied_prover:proves([a] => [a], 0, _, _), perturbation(resource_exhaustion), true))
    )).
...
test_pml_dynamics :-
    ...
    run_test('Dialectical rhythm: U -> Box_down(A)', (
        embodied_prover:proves([s(u)] => [s(comp_nec a)], 50, _, _)
    )),

    run_test('Dialectical rhythm: A -> Diamond_up(LG)', (
        embodied_prover:proves([s(a)] => [s(exp_poss lg)], 50, _, _)
    )),
    ...
    run_test('Bad Infinite: Being <-> Nothing', (
        embodied_prover:proves([s(t_b)] => [s(comp_nec t_n)], 50, _, _),
        embodied_prover:proves([s(t_n)] => [s(comp_nec t_b)], 50, _, _)
    )),

    run_test('Oobleck Dynamic: S -> O transfer', (
        embodied_prover:proves([s(comp_nec p)] => [o(comp_nec p)], 50, _, _)
    )),

    run_test('Modal context switch: compressive', (
        embodied_prover:proves([s(u)] => [s(comp_nec a)], 100, R_Out, _),
...
test_trace_mechanism :-
    ...
    run_test('Elusive Subject axiom: S-O inversion', (
        pragmatic_axioms:i_feeling(I_f),
        embodied_prover:proves([s(comp_nec I_f)] => [o(exp_nec I_f)], 50, _, _)
    )),
    ...
    run_test('Proof erasure: trace propagation', (
        pragmatic_axioms:i_feeling(I_f),
        embodied_prover:proves([s(I_f)] => [s(I_f)], 50, _, Proof),
        Proof = erasure(_)
    )).

...
execute_strategy(Num1, /, Num2, 'IDP', Result, History) :-
    % No hardcoded KB — IDP must rely on learned multiplication facts.
    % This makes the elaboration chain (add → multiply → divide) real.
    smr_div_idp:run_idp(Num1, Num2, [], Result, History).

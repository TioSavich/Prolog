...
def _canonical_event(
    ...
    return {
        ...
        "pml": {
            ...
            "polarity": polarity,
            "force": "binding",
            "operator": "comp_nec" if polarity == "compressive" else "possibility",
...
def _material_handles(topic: str, polarity: str) -> list[dict[str, str]]:
    ...
    return [
        {
            "handle": handle,
            "metaphor": _metaphor_for_topic(topic),
            "polarity": polarity,
        }
    ]
...
def _metaphor_for_topic(topic: str) -> str:
    if topic in {"inclusive_shape_hierarchy", "prototype_vs_definition"}:
        return "category_as_container"
    if topic in {"measure_vs_object", "angle_as_turn_vs_length"}:
        return "measurement_as_path"
    return "inference_as_transition"
